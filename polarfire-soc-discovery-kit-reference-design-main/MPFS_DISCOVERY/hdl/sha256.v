   ...
   localparam logic [31:0] K[0:63] = '{
    ...
    always_ff @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            // ---------------- accept input blocks ----------------
            if (!end_seen && !buffer_full && valid_i) begin
                buffer      <= block_i;
                buffer_full <= 1'b1;
            end
            ...
            case(state)
                ...
                COMPUTE: begin
                    ...
                    for (j =  0; j < 16 - DEPTH; j++) W[j] <= W[j + DEPTH];    
                    for (j = 16 - DEPTH; j < 16; j++) W[j] <= W_next[j - (16 - DEPTH)];
                    
                    if (t >= 64 - DEPTH) state <= UPDATE; // finished the block
                    else                 t     <= t + DEPTH;  // next chunk of rounds
                end
